      ...
      >
        ...
        <Avatar_look
          :user="{
            ...
            username: room.other_user_info.name,
            user_avatar: room.other_user_info.avatar
          }"
          size="medium"
        />
        ...
        <div class="chat-info">
          <div class="chat-header">
            <span class="chat-username">{{ getOtherUserInfo(room)?.name }}</span>
            <!-- 未读消息计数 -->
            <span
...
import type { PrivateChatRoom, ChatUserInfo } from '../../api/chat';
...
// 获取聊天房间中的对方用户信息
const getOtherUserInfo = (room: PrivateChatRoom): ChatUserInfo | undefined => {
  return room.other_user_info;
};
...
const getLastMessage = (roomId: number): string => {
  ...
  // 检查是否为视频消息
  if (lastMessage.messages_type === 'video') {
    return '[视频消息]';
  }
  
  // 检查是否为文件消息
  if (lastMessage.messages_type === 'file') {
...
const sortedChatRooms = computed(() => {
  return [...chatStore.privateChatRooms].sort((a, b) => {
